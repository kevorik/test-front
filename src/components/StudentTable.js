...
import { getStudents, createStudent, getClasses, deleteStudent, updateStudent } from '../services/api';
import {
Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Typography, Button, TextField, Box, MenuItem, Select, InputLabel, FormControl, Dialog, DialogActions, DialogContent, DialogTitle
} from '@mui/material';
...
class: { id: newStudent.class.id }
  ...
  if (response.status === 201) {
    // Assuming the student was created successfully
    getStudents().then(response => setStudents(response.data)); // Refresh the student list
    setShowCreateStudentForm(false);
    setNewStudent({ first_name: '', last_name: '', middle_name: '', class: { id: '' } }); // Reset the form fields
  }
} catch (error) {
...
<TableCell>{student.class ? student.class.name : 'N/A'}</TableCell>
<TableCell>
<Button
variant="contained"
color="secondary"
...
<Box mt={2}>
<Button variant="contained" color="primary" onClick={() => setShowCreateStudentForm(true)}>
Create Student
</Button>
</Box>
{showCreateStudentForm && (
<Box mt={2}>
<Typography variant="h6" gutterBottom>Create New Student</Typography>
<TextField
label="First Name"
...
onChange={(e) => setNewStudent({ ...newStudent, class: { id: e.target.value } })}
>
...
<Button variant="contained" color="secondary" onClick={() => setShowCreateStudentForm(false)} style={{ marginLeft: '10px' }}>
Cancel
</Button>
</Box>
</Box>
  ...
  {showEditStudentForm && selectedStudent && (
    <Dialog open={showEditStudentForm} onClose={() => setShowEditStudentForm(false)}>
      ...
      <DialogContent>
        ...
        />
        <FormControl fullWidth margin="normal">
          <InputLabel id="editClassId-label">Class</InputLabel>
          <Select
            labelId="editClassId-label"
            value={selectedStudent.class.id}
            onChange={(e) => setSelectedStudent({ ...selectedStudent, class: { id: e.target.value } })}
          >
            {classes.map((classItem) => (
              <MenuItem key={classItem.id} value={classItem.id}>
                {classItem.name}
      ...
      <DialogActions>
        <Button onClick={() => setShowEditStudentForm(false)} color="secondary">
          Cancel
        </Button>
