...
import { getClasses, createClass, getTeachers, getStudents, getSchools, deleteClass, updateClass } from '../services/api';
import { 
  Table, 
  TableBody, 
  TableCell, 
...
const ClassTable = () => {
  ...
  const handleChange = (event) => {
    ...
    if (name === "school") {
      ...
      updatedValue = { id: selectedTeacher.id, name: `${selectedTeacher.first_name} ${selectedTeacher.last_name}` };
    } else if (name === "classPrefect") {
      const selectedPrefect = students.find(student => student.id === value);
      updatedValue = { id: selectedPrefect.id, name: `${selectedPrefect.first_name} ${selectedPrefect.last_name}` };
    }
  ...
  const openEditModal = (classItem) => {
    setEditClassData({
      ...
      classTeacher: { id: classItem.classTeacher.id, name: `${classItem.classTeacher.first_name} ${classItem.classTeacher.last_name}` },
      classPrefect: { id: classItem.classPrefect.id, name: `${classItem.classPrefect.first_name} ${classItem.classPrefect.last_name}` }
    });
    ...
    <div>
          ...
          <TableBody>
            {classes.map((classItem) => (
              <TableRow key={classItem.id}>
                ...
                <TableCell>{classItem.school ? classItem.school.name : 'N/A'}</TableCell>
                <TableCell>{classItem.classTeacher ? `${classItem.classTeacher.first_name} ${classItem.classTeacher.last_name}` : 'N/A'}</TableCell>
                <TableCell>{classItem.classPrefect ? `${classItem.classPrefect.first_name} ${classItem.classPrefect.last_name}` : 'N/A'}</TableCell>
                <TableCell> {classItem.students && classItem.students.length > 0 ? (
                  <ul>
      ...
      {showCreateForm && (
        <Box mt={2}>
          ...
          <FormControl fullWidth margin="normal">
            <InputLabel id="classTeacherId-label">Class Teacher</InputLabel>
            <Select
              labelId="classTeacherId-label"
              value={newClass.classTeacher.id}
              onChange={(e) => setNewClass({ ...newClass, classTeacher: { id: e.target.value } })}
            >
          ...
          <FormControl fullWidth margin="normal">
            <InputLabel id="classPrefectId-label">Class Prefect</InputLabel>
            <Select
              labelId="classPrefectId-label"
              value={newClass.classPrefect.id}
              onChange={(e) => setNewClass({ ...newClass, classPrefect: { id: e.target.value } })}
            >
      ...
      <Modal open={showEditModal} onClose={() => setShowEditModal(false)}>
        ...
        >
          ...
          <Box mt={2}>
            <Button variant="contained" color="primary" onClick={handleEditClass}>
              Save
            </Button>
            <Button
              variant="contained"
